...
/// A model regression problem solver.
/// It needs to be trained before it can make predictions.
/// TODO: Add configuration size parameter
/// TODO: Update tests
/// TODO: Update doctests
/// TODO: Refactor
/// TODO: Update documentation
#[derive(Debug, PartialEq, Clone)]
pub struct Solver<T : Float, F : Fn(&Matrix<T>, &Matrix<T>) -> T> {
...
impl<T : Float, F : Fn(&Matrix<T>, &Matrix<T>) -> T> Solver<T, F> {
    /// Creates a new Solver with an empty configuration.
    pub fn new(hypothesis: F) -> Solver<T, F> {
        Solver { configuration: None, hypothesis: hypothesis }
    }
    ...
    fn train_gradient_descent_feature_scaling(hypothesis: &F, n_inputs: &Matrix<T>, outputs: &Matrix<T>) -> Option<Matrix<T>> {
        ...
        let mut configuration = Solver::<T, F>::train_gradient_descent(hypothesis, &scaled_n_inputs, outputs)?;
    ...
    pub fn train(&mut self, inputs: &Matrix<T>, outputs: &Matrix<T>) -> bool {
        ...
        self.configuration = Solver::<T, F>::train_gradient_descent_feature_scaling(&self.hypothesis, &n_inputs, outputs);
...
mod tests {
    ...
    fn train_gradient_descent() {
        for i in 0..tests_inputs().len() {
            let mut solver = Solver::new(|configuration, mat| (&configuration.transpose() * &mat.transpose())[(0, 0)]);
    ...
    fn run() {
        for i in 0..tests_inputs().len() {
            let solver = Solver { configuration: Some(tests_configuration()[i].clone()), hypothesis: |configuration, mat| (&configuration.transpose() * &mat.transpose())[(0, 0)]};
