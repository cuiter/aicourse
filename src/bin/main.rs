use aicourse::matrix::{Matrix, Float, load_idx};
...
fn main() {
    ...
    let mut network = NeuralNetwork::<f32>::new(vec![28 * 28, 20, 10]);
    ...
    println!("Train accuracy: {}", accuracy(&network.run(&train_images), &train_labels));
    println!("Test accuracy: {}", accuracy(&network.run(&t10k_images), &t10k_labels));
}
